...
function scheduleHistorySave() {
  ...
  historySaveTimer = setTimeout(async () => {
    ...
    try {
      ...
      historyCache = filtered.slice(-1000);
      historyDirty = false;
...
function serveFile(res, filePath, downloadName = null, forceDownload = false) {
  ...
  // CSS/JS/图片等静态资源明确设置为 inline，防止 cloudflare tunnel 添加 attachment
...
server.on("upgrade", (req, socket, head) => {
  ...
  if (urlPath.startsWith("/v1/gateway") || urlPath === "/ws") {
    ...
    socket.on('close', (hadError) => {
      MetricsCollector.removeWsConnection();
      console.log(`[WS] Connection closed. Total: ${MetricsCollector.wsConnections}`);

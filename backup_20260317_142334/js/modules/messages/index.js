...
class MessagesModule {
    ...
    updateStreamingMessage(runId, text, sender, senderAgentId, model, thinking = null, tools = []) {
        ...
        if (msgEl) {
            // 更新思考块
            if (thinking) {
                let thinkingEl = msgEl.querySelector('.thinking-block');
                if (!thinkingEl) {
                    thinkingEl = document.createElement('div');
                    const contentWrapper = msgEl.querySelector('.msg-content-wrapper');
                    const contentEl = msgEl.querySelector('.msg-content');
                    if (contentWrapper && contentEl) {
                        contentWrapper.insertBefore(thinkingEl, contentEl);
                    }
                }
                thinkingEl.outerHTML = renderThinkingBlock(thinking, true);
            }
            
            // 更新工具卡片
            if (tools && tools.length > 0) {
                let toolsSection = msgEl.querySelector('.tools-section');
                if (!toolsSection) {
                    ...
                    const contentEl = msgEl.querySelector('.msg-content');
                    if (contentWrapper && contentEl) {
                        contentWrapper.insertBefore(toolsSection, contentEl.nextSibling);
                    }
                }
                toolsSection.innerHTML = tools.map(tool => {
                    if (tool.type === 'tool_use') {
                        return renderToolCard(tool.name, tool.params, tool.status, null);
                    } else if (tool.type === 'tool_result') {
                        return renderToolCard('工具结果', null, tool.status, tool.result);
                    }
                    return '';
                }).join('');
            }
    ...
    finalizeStreamingMessage(runId, text, model, thinking = null, tools = []) {
        ...
        if (msgEl) {
            ...
            if (thinking) {
                ...
                thinkingEl.outerHTML = renderThinkingBlock(thinking, false);
            }

...
class App {
    ...
    _respondToChallenge(nonce) {
        ...
        // 正确做法：收到 challenge 后发送 connect 请求（带 token）
        const connectReq = {
            type: 'req',
            id,
            method: 'connect',
            params: {
                ...
                auth: { token: this.authToken, nonce: nonce },
                role: 'operator',
                scopes: ['operator.admin', 'operator.write', 'operator.read']
            }
        };

        console.log('[App] Responding to challenge with connect request (with nonce)');
        this.ws.send(JSON.stringify(connectReq));
    }
    ...
    _sendToAgentNow(agentId, message) {
        ...
        // 获取或创建会话 ID
        let sessionId = state.getStateValue('currentSessionId');
        if (!sessionId) {

...
class SidebarModule {
    ...
    renderSessionList() {
        ...
        const recentCount = sessions.filter((session) => (Date.now() - (session.lastMessageTime || 0)) < recentWindowMs).length;
            ...
            if (!this.sessionListEl) {
                ...
                sessionSection.innerHTML = `
                    <div class="session-header">
                        <span>📋 活跃会话</span>
                        <span class="session-header-stats">0 / 0</span>
                    </div>
        ...
        const sessionHeaderStats = this.container?.querySelector('.session-header-stats');
        if (sessionHeaderStats) {
            sessionHeaderStats.textContent = `${recentCount} / ${sessions.length}`;
            sessionHeaderStats.title = '最近 30 分钟活跃会话 / 总会话数';
        }
        ...
        this.sessionListEl.innerHTML = sessions.slice(0, 10).map(session => {
            ...
            const compactId = getCompactSessionId(session.id);
                ...
                <div class="session-item" data-session-id="${session.id}">
                    ...
                    <div class="session-info">
                        ...
                        <div class="session-meta">
                            <span class="session-id">#${compactId}</span>
